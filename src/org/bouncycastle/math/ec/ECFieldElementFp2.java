...
public class ECFieldElementFp2 extends ECFieldElement {
     ...
     */
    private ECFieldElement.Fp c1, c0;
     ...
     */
    public ECFieldElement.Fp getCoeff1() {
	return c1;
    }
     ...
     */
    public ECFieldElement.Fp getCoeff0() {
	return c0;
    }
	...
	if (p.signum() == 1 && p.mod(FOUR).equals(THREE)) {
	    c1 = new ECFieldElement.Fp(p, x.divide(p));
	    c0 = new ECFieldElement.Fp(p, x.mod(p));
	} else {
	...
	if (check) {
	    if (c0 instanceof ECFieldElement.Fp
		    && c1 instanceof ECFieldElement.Fp
		    && ((ECFieldElement.Fp) c0).getQ().equals(
			    ((ECFieldElement.Fp) c1).getQ())
		    && ((ECFieldElement.Fp) c0).getQ().signum() == 1
		    && ((ECFieldElement.Fp) c0).getQ().mod(FOUR).equals(THREE)) {
		this.c1 = (ECFieldElement.Fp) c1;
		this.c0 = (ECFieldElement.Fp) c0;
	    } else {
     ...
     * 
     */
    public ECFieldElementFp2(ECFieldElement.Fp f) {
	c1 = new ECFieldElement.Fp(f.getQ(), ZERO);
	c0 = f;
    }
	...
	return new ECFieldElementFp2(new ECFieldElement.Fp(p, c1.toBigInteger()
		.multiply(scalar).mod(p)), new ECFieldElement.Fp(p, c0
	...
	if (this.isZero()) {
	    ...
	    BigInteger p = this.getPrime();
	    ECFieldElement.Fp _0 = new ECFieldElement.Fp(p, ZERO);
	    ECFieldElement.Fp _1 = new ECFieldElement.Fp(p, ONE);
	...
	ECFieldElement.Fp _1 = new ECFieldElement.Fp(this.getPrime(), ONE);
	...
	ECFieldElement r = new ECFieldElementFp2(new ECFieldElement.Fp(p, D1),
		new ECFieldElement.Fp(p, D0), false);
